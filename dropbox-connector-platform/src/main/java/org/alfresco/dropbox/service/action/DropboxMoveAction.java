    ...
    extends ActionExecuterAbstractBase
{
    ...
    protected void executeImpl(final Action action, final NodeRef actionedUponNodeRef)
    {
        ...
        Map<String, NodeRef> syncedUsers = dropboxConnector.getSyncedUsers(((ChildAssociationRef)action.getParameterValue(DROPBOX_TO_PATH)).getChildRef());
        ...
        for (final Map.Entry<String, NodeRef> syncedUser : syncedUsers.entrySet())
        {
            AuthenticationUtil.runAs(new AuthenticationUtil.RunAsWork<Metadata>()
            {
                public Metadata doWork()
                    throws Exception
                {
                    Metadata metadata = dropboxConnector.move((ChildAssociationRef)action.getParameterValue(DROPBOX_FROM_PATH), (ChildAssociationRef)action.getParameterValue(DROPBOX_TO_PATH));
                    dropboxConnector.persistMetadata(metadata, ((ChildAssociationRef)action.getParameterValue(DROPBOX_TO_PATH)).getChildRef());
