...
import org.alfresco.dropbox.exceptions.*;
    ...
    extends Node
{
            ...
            for (NodeRef nodeRef : nodeRefs)
            {
                if (nodeService.exists(nodeRef))
                {
                    ...
                    catch (FileExistsException fee)
                    {
                        ...
                        Metadata metadata = dropboxConnector.getMetadata(fee.getNodeRef());

                        dropboxConnector.persistMetadata(metadata, fee.getNodeRef());
                    }
                }
    ...
    private void add(NodeRef nodeRef)
    {
        ...
        if (nodeService.getType(nodeRef).equals(ContentModel.TYPE_CONTENT))
        {
            metadata = dropboxConnector.putFile(nodeRef, false);
        }
        else if (nodeService.getType(nodeRef).equals(ContentModel.TYPE_FOLDER))
        {
            try
                ...
                if (nodeService.exists(fileInfo.getNodeRef()))
                {
                    try
                    {
                        ...
                        else if (nodeService.getType(fileInfo.getNodeRef()).equals(ContentModel.TYPE_FOLDER))
                        {
                            ...
                            Metadata metadata = dropboxConnector.getMetadata(fileInfo.getNodeRef());
                            dropboxConnector.persistMetadata(metadata, fileInfo.getNodeRef());
                        }
                    }
                    catch (FileExistsException fee)
                    {
    ...
    private void makeSyncable(NodeRef nodeRef)
    {
        ...
        if (nodeRef != null)
        {
            ...
            {
                ChildAssociationRef childAssocRef = nodeService.createNode(nodeRef, DropboxConstants.Model.ASSOC_SYNC_DETAILS, DropboxConstants.Model.DROPBOX, DropboxConstants.Model.TYPE_STATUS);
                nodeService.setProperty(childAssocRef.getChildRef(), DropboxConstants.Model.PROP_SYNCING, false);
        ...
        {
            throw new WebScriptException(nodeRef + " is not present in the repository.");
        }
    }
